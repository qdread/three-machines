...
### sbatch --job-name=newprior3m --ntasks=4 --mem=4gb --partition=medium --time=7-00:00:00 --export=scriptname="soybean_machine_comparison/three_machine_fit_latent_newpriors.R" runjob.sh
...
fit_latent <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(TRUE), data = dat_wide,
                  ...
                  file = file.path(fp, 'three_machine_fit_latent_gammaprior'))
...
fit_latent_norescor <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(FALSE), data = dat_wide,
                  ...
                  control = list(adapt_delta = 0.95, max_treedepth = 20),
                  file = file.path(fp, 'three_machine_fit_latent_gammaprior_norescor'))
