...
# Yet another version, where the variance of the latent variable is fixed at 1, which constrains 
# the model so that it will be identifiable.
...
### sbatch --job-name=setvarconst --ntasks=4 --mem=4gb --export=scriptname="soybean_machine_comparison/three_machine_fit_latent_setvarconstant.R" runjob.sh
...
fit_latent_rescor_varconstant <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(TRUE), data = dat_wide,
                  prior = latent_model_priors,
                  chains = 4, iter = 10000, warmup = 7500, seed = 91919, stanvars = stanvars,
                  control = list(adapt_delta = 0.95, max_treedepth = 20),
                  file = file.path(fp, 'three_machine_fit_latent_rescor_varconstant'))

fit_latent_norescor_varconstant <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(FALSE), data = dat_wide,
                  prior = latent_model_priors,
                  chains = 4, iter = 10000, warmup = 7500, seed = 19191, stanvars = stanvars,
                  control = list(adapt_delta = 0.95, max_treedepth = 20),
                  file = file.path(fp, 'three_machine_fit_latent_norescor_varconstant'))
