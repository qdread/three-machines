...
### sbatch --job-name=set2sigconst --ntasks=4 --mem=4gb --export=scriptname="three_machine_fit_latent_set2sigmaconstant.R" runjob.sh
...
latent_model_priors <- c(
  ...
  prior(constant(1), class = sigma, resp = Vol),
  prior(constant(1), class = sigma, resp = TWtrue)
)

fit_latent_rescor_2sigmaconstant <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(TRUE), data = dat_wide,
                  prior = latent_model_priors,
                  chains = 4, iter = 10000, warmup = 7500, seed = 65432, stanvars = stanvars,
                  control = list(adapt_delta = 0.95, max_treedepth = 20),
                  file = file.path(fp, 'three_machine_fit_latent_rescor_2sigmaconstant'), file_refit = 'never')

fit_latent_norescor_2sigmaconstant <- brm(bf_gac + bf_vol + bf_per + bf_tw + set_rescor(FALSE), data = dat_wide,
                  prior = latent_model_priors,
                  chains = 4, iter = 10000, warmup = 7500, seed = 23456, stanvars = stanvars,
                  control = list(adapt_delta = 0.95, max_treedepth = 20),
                  file = file.path(fp, 'three_machine_fit_latent_norescor_2sigmaconstant'), file_refit = 'never')
